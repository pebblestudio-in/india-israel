...
const FILES = ["core","timeline","communities","people","articles","cities","languages","statements","synagogues","corrections"];
const data = {};
for (const f of FILES) {
  const j = JSON.parse(fs.readFileSync(`content/${f}.json`, "utf8"));
  if (["articles","cities","languages","statements","synagogues","corrections"].includes(f)) data[f] = j;
...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,300..700;1,9..144,300..700&family=Archivo:wght@400;500;600&family=IBM+Plex+Mono:wght@400;500&family=Noto+Sans+Hebrew:wght@400;600&family=Noto+Sans+Devanagari:wght@400;600&display=swap">
<style>${css}</style>
${markup}
<script>${cfg}</script>
